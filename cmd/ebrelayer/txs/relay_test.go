...
// 	//t *testing.T, validatorAddress
// 	//sdk.AccAddress,
// 	//testEthereumAddress string,
// 	//coins string
// )EthBridgeClaim {
...
func TestRelayMsg(t *testing.T) {
	//Setup
	cdc := codec.New()
	// (sdk.Context, auth.AccountKeeper, Keeper, bank.Keeper, []sdk.ValAddress, sdk.Error)
	...
	handler := ethbridge.NewHandler(keeper, bankKeeper, cdc, types.DefaultCodespace)

	//Claim Creation
	testClaim := types.CreateTestEthMsg(t, accAddress)
	res := handler(ctx, testClaim)
	require.True(t, res.IsOK())

	//Attempt to relay the claim
	relayResult := RelayEvent(ChainId, cdc, accAddress, ValidatorName, ValidatorPassword, testClaim)

	//Unrecognized type
	// res := handler(ctx, sdk.NewTestMsg())
	require.False(t, strings.Contains(relayResult.Log, "invalid"))
