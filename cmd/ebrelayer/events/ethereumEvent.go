...
// UnpackLogLock : Unpacks new LogLock events and returns a LockEvent
func UnpackLogLock(
	...
	// Declare new LockEvent struct
	event := LockEvent{}

	// Populate Bridge contract address and Ethereum chain ID field
	if !common.IsHexAddress(bridgeContractAddress) {
		log.Fatalf("Only Ethereum contracts are currently supported. Invalid address: %v", bridgeContractAddress)
	}
	event.BridgeContractAddress = common.HexToAddress(bridgeContractAddress)
